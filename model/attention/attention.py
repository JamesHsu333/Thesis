...
class PAM_without_filter_gamma(nn.Module):
    """ Position attention module without 1x1 conv"""
    def __init__(self, gamma):
        super(PAM_without_filter_gamma, self).__init__()
        self.softmax = nn.Softmax(dim=-1)
        self.gamma = gamma
        self._init_weight()
    def forward(self, x):
...
def build_PAM_without_filter_gamma(gamma):
    return PAM_without_filter_gamma(gamma)